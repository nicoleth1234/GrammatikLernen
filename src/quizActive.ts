...
const KASUS_ORDER: Kasus[] = [Kasus.Nom, Kasus.Gen, Kasus.Dat, Kasus.Akk, Kasus.Abl, Kasus.Vok];

// ---------- Parser ----------
function parseDeklination(v: string): Deklinationen {
    switch (v) {
...
function parseGenus(v: string): Genus {
    switch (v) {
...
function parseNumerus(v: string): Numerus {
    switch (v) {
...
function parseKasus(v: string): Kasus {
    switch (v) {
...
async function ladeAlle(): Promise<Deklination[]> {
    ...
    for (const [k, v] of Object.entries(i)) if (v === -1) throw new Error(`CSV-Header fehlt Spalte: ${k}`);

    return csv.slice(1).map(r => new Deklination(
        parseDeklination(r[i.Deklination]),
        parseKasus(r[i.Kasus]),
        parseGenus(r[i.Genus]),
        parseNumerus(r[i.Numerus]),
        r[i.Endung].trim()
    ));
}

// ---------- Quiz ----------
type Frage = { dekl: Deklinationen; genus: Genus; kasus: Kasus; numerus: Numerus; antwort: string };

class QuizEngine {
    private pool: Frage[] = [];
    private current?: Frage;

    constructor(daten: Deklination[]) {
        // erlaubte Kombinationen: a(f), o(m), o(n), 3(m), 3(n)
        const erlaubt = daten.filter(d =>
            (d.deklination === Deklinationen.A && d.genus === Genus.F) ||
    ...
    getCurrent(): Frage | undefined { return this.current; }
}
