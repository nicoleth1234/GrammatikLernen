...
const withVoc = ((_a = params.get("voc")) !== null && _a !== void 0 ? _a : "on") === "on";
const mode = ((_b = params.get("mode")) !== null && _b !== void 0 ? _b : "normal");
// Zeilenwahl je nach Vokativ-Flag
const ROWS = withVoc ? KASUS_ORDER : [Kasus.Nom, Kasus.Gen, Kasus.Dat, Kasus.Akk, Kasus.Abl];
// --- Helpers ---
function norm(s) {
...
function buildCards(forms) {
    ...
    for (const c of list) {
        ...
        chip.addEventListener("dragstart", (ev) => {
            var _a;
            (_a = ev.dataTransfer) === null || _a === void 0 ? void 0 : _a.setData("text/plain", c.text);
...
function onDropCell(ev, td) {
    ...
    if (!dropped)
        return;
    const key = `${td.dataset.kasus}|${td.dataset.numerus}`;
    ...
    if (!td.classList.contains("filled")) {
        ...
        if (mode === "normal") {
            ...
            const card = Array.from(poolEl.children).find(el => norm(el.dataset.value || "") === norm(dropped));
            if (card)
                card.remove();
        }
        else {
