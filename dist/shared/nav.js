...
function detectProjectRoot(pathname) {
    const m = pathname.match(/^(.*?\/)pages\/(latin|greek)\//);
    if (m)
        return m[1]; // .../deklinationen/
...
export function wireBackLinkToQuiz(selector = 'a[href$="quiz.html"]') {
...
export function goSibling(file) {
...
export function goSiblingTo(file, search = "") {
    const url = new URL(window.location.href);
...
export function wireBackLinkToGreekNounsMenu(selector = 'a[href$="quiz-gr-nouns-menu.html"]') {
    const a = document.querySelector(selector);
    if (!a)
        return;
    a.addEventListener("click", (ev) => {
